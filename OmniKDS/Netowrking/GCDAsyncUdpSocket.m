...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        // Updatesself->flags, add connect packet to send queue, and pump send queue
    ...
    if (sendFilterBlock && sendFilterQueue)
    {
        ...
        else
        {
            ...
            dispatch_sync(sendFilterQueue, ^{ @autoreleasepool {
                ...
                allowed = self->sendFilterBlock(currentSend->buffer, currentSend->address, currentSend->tag);
            }});
    ...
    else
    {
        ...
        if (socket4FDBytesAvailable > 0)
        {
            if (socket6FDBytesAvailable > 0)
            {
                ...
                flags ^= kFlipFlop; //self->flags =self->flags xor kFlipFlop; (toggle flip flop bit)
            }
            else {
...
- (void)closeWithError:(NSError *)error
{
    ...
    // Clear allself->flags (config remains as is)
    flags = 0;
